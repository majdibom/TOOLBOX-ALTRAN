...
public interface RiskRepository extends JpaRepository<Risk, Long> {
	...
	@Query("SELECT DISTINCT t FROM Risk t where ((t.probability in (:probabilities))"
			...
			+ " OR ((:riskNatures) is null))  AND ((t.exposure in (:exposures))"
			+ " OR ((:exposures) is null)) AND ((t.riskStatus in (:status))"
			+ " OR ((:status) is null)) ")
	public Page<Risk> advancedSearch(@Param("probabilities") Set<Probability> probabilities,
			@Param("riskNatures") Set<RiskNature> riskNatures, @Param("exposures") Set<Exposure> exposures,
			@Param("status") Set<RiskStatus> status, Pageable pageable);
}
